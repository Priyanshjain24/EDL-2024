...
if __name__ == '__main__':
    ...
    while(1):
        ...
        deltaRx = sum([abs(rxValues[i] - prev_rxValues[i]) for i in range(4)])
        if deltaRx > 2 :
            rxValues = prev_rxValues
        prev_rxValues = rxValues
        print(f"{rxValues} Delta : {deltaRx}")
        ...
        # print(f"{angle_values}, X = {X:.2f}, Y = {Y:.2f}")
